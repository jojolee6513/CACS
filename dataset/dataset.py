...
#     X_train, y_train = dataset_to_numpy(train_dataset)#X_train=train_data.mat
#     X_test, y_test = dataset_to_numpy(test_dataset)#y_train=train_label.mat
...
def load_data_HSI(name, seed, numComponents, window_size, train_ratio, imbalance=None, data_dir=None):
    ...
    n_classes = len(np.unique(y_test))
    print(X_train.shape, y_train.shape, X_test.shape, y_test.shape)
    tpm=[np.count_nonzero(y_train == i) for i in range(n_classes)]
    print(tpm)
    if imbalance is None or imbalance is False:
        return (X_train, y_train), (X_test, y_test)
    if imbalance is True:
    ...
    print(X_train.shape, y_train.shape, X_test.shape, y_test.shape)
    return (X_train, y_train), (X_test, y_test), test_indices, gt_IN, tpm
